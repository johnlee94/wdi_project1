...
function clearOnReturn(event) {
    ...
    if (event.keyCode === 13) {
        ...
        powerControl();
        powerTenChecker = 0;
        if (powerTenChecker === 0 && !warning10) {
            powerLessTen();
            powerTenChecker = 1;
        }
    ...
    if (x === '.help') {
        ...
        $('#log').text("LIST OF COMMANDS-- .esc, .help, .map, .cam, .cam1, .cam2, .cam3, .cam4, .cam5, .cam6, .cam7, .cam8, .lock, .unlock, .open, .close, .login, .music");
    } else if (x === '.map') {
        $('#mgw').text('')
        //code to open map
...
function powerLessTen() {
    if (power < 10 && power > 0) {
        ...
        $('#log').text("WARNING: Less than 10% Power remaining.")
